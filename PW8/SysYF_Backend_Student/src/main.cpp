...
int main(int argc, char *argv[])
{
    ...
    bool register_optimize = false;
    bool moving_optimize = false;
    ...
    for (int i = 1; i < argc; ++i) {
        if (argv[i] == std::string("-h") || argv[i] == std::string("--help")) {
            ...
            register_optimize = true;
        }
        else if (argv[i] == std::string("-m")){
            moving_optimize = true;
        }
        else {
            filename = argv[i];
        }
    ...
    if (emit_ir || emit_assembly) {
        ...
        if(emit_assembly){
            ...
            if (register_optimize){
                coder.register_optimize = true;
            }
            if (moving_optimize){
                coder.moving_optimize = true;
            }
